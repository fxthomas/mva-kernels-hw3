...
gamma_values_sigmoid = exp (arange (-7, 2, 1))
degree_values = arange (1, 5, 1)
error_train_values = zeros (C_values.shape)
error_test_values = zeros (C_values.shape)

phi = get_cmap('jet')
color_count = len(gamma_values) + len(gamma_values_sigmoid) + len(degree_values) + 1
colors = [phi(float(i)/float(color_count)) for i in range(color_count)]
...
for j in range(len(gamma_values_sigmoid)):
  for i in range(len(C_values)):
    gamma = gamma_values_sigmoid[j]
    C = C_values[i]
    print "  --> Sigmoid, Gamma = {0} ; C = {1}".format (gamma, C)

    # Train SVM
    model = svm.SVC (C=C, gamma=gamma, kernel='sigmoid')
    model.fit (data_train, classes_train)
    ...
    error_train_values[i] = float(len(find(predictions_train != classes_train)))/float(len(classes_train))
    error_test_values[i]  = float(len(find(predictions_test != classes_test)))/float(len(classes_test))
    print "       {0}".format (error_test_values[i])

  semilogx (C_values, error_test_values, color=colors[j+len(degree_values)+len(gamma_values)], label="Sigmoid, g={0:.3f}".format(gamma), linewidth=3.0)
